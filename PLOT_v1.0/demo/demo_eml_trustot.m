...
% the trust optimization transfer algorithm.
%
...
% set regularization
R = buildNbhd(imgsiz, 'clique', 2);
R = buildPenalty(imgsiz, 'hyper', delta, 1, 0, R);
...
for i = 1:length(x)
    Li = (L{i}-L0)/(L{i}(1)-L0);
    semilogy(1:length(Li),Li,lmark{i}, 'LineWidth', 2); hold on;
end
...
figure, 
for i = 1:length(x)
    plot(out{i}.time, snr{i},lmark{i}, 'LineWidth', 3); hold on;
end
set(gca,'FontSize',16); xlabel('CPU time (s)'); ylabel('image SNR (dB)');
title('PL reconstruction'); legend(lname{1}, lname{2}, lname{3}, 'Location', 'Best')
